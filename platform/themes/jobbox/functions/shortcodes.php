...
app()->booted(function () {
    ...
    if (is_plugin_active('job-board')) {
        add_shortcode('search-box', __('Search box'), __('The big search box'), function ($shortcode) {
            ...
            // Retrieve featured companies (similar to the top-companies page)
            $featureCompanies = app(CompanyInterface::class)
                ->advancedGet([
                    'with' => $with,
                    'condition' => [
                        'is_featured' => 1,  // Ensure featured companies are retrieved
                        'status' => BaseStatusEnum::PUBLISHED,  // Only get published companies
                    ],
                    'withCount' => [
                        'reviews',
                        'jobs',
                    ...
                    'take' => (int)$shortcode->limit_company ?: 10,  // Set the limit for the number of companies to show
                    'order_by' => ['created_at' => 'DESC'],
                ]);

            // Depending on the style, render the appropriate data
            ...
            // Add consultants data for style-1 or default case
            $consultants = Account::where('type', 'consultant')
                ->with('consultantReviews')
                ->select(['id', 'first_name', 'last_name', 'address', 'description'])
                ->limit(10)->latest()->get();

            return Theme::partial('shortcodes.search-box', compact('shortcode', 'consultants', 'featureCompanies'));
        });
        ...
        add_shortcode('featured-job-categories', __(' categories'), __('Featured job categories'), function ($shortcode) {
            $categories = app(CategoryInterface::class)
