...
use Botble\JobBoard\Facades\CurrencyFacade;
    ...
    function format_price($price, $currency = null, $withoutCurrency = false, $useSymbol = true): string
    {
        if ($currency) {
            if (! $currency instanceof Currency) {
                $currency = app(CurrencyInterface::class)->getFirstBy(['jb_currencies.id' => $currency]);
            }
...
if (! function_exists('human_price_text')) {
    ...
    function human_price_text($price, $currency, $priceUnit = ''): string
    {
        ...
        if (setting('job_board_convert_money_to_text_enabled', false)) {
