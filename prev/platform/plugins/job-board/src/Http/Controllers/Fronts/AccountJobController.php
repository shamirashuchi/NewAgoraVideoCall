...
class AccountJobController extends Controller
{
    ...
    public function store(AccountJobRequest $request, BaseHttpResponse $response)
    {
        ...
        if ($job->status == BaseStatusEnum::PUBLISHED) {
    ...
    public function update(int $id, AccountJobRequest $request, BaseHttpResponse $response)
    {
        $job = $this->jobRepository->findOrFail($id);
        ...
        if ($job->status != BaseStatusEnum::PUBLISHED && $request->input('status') == BaseStatusEnum::PUBLISHED) {
            $this->sendEmailAboutNewJobToAdmin($job);
        }
    ...
    protected function processRequestData(Request $request): Request
    {
        if ($request->hasFile('featured_image_input')) {
            $result = RvMedia::handleUpload($request->file('featured_image_input'), 0, 'accounts');
            if (! $result['error']) {
                $file = $result['data'];
